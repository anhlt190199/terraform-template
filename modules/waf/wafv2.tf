...
resource "aws_wafv2_web_acl" "main" {
  ...
  name  = format("%s-%s", var.master_prefix, var.waf_name)
  scope = var.scope
    ...
    content {
                  ...
                  content {
                    statement {
                      ...
                      dynamic "ip_set_reference_statement" {
                        ...
                        content {
                          arn = aws_wafv2_ip_set.ip_set[lookup(ip_set_reference_statement.value, "ip_set_index")].arn
                        }
                      }
      ...
      dynamic "visibility_config" {
        ...
        content {
          ...
          metric_name                = lookup(visibility_config.value, "metric_name", "${var.master_prefix}-default-rule-metric-name")
  ...
  dynamic "visibility_config" {
    ...
    content {
      cloudwatch_metrics_enabled = lookup(visibility_config.value, "cloudwatch_metrics_enabled", true)
      metric_name                = lookup(visibility_config.value, "metric_name", "${var.master_prefix}-default-web-acl-metric-name")
      sampled_requests_enabled   = lookup(visibility_config.value, "sampled_requests_enabled", true)
    }
  }
}
...
resource "aws_wafv2_web_acl_association" "alb_list" {
  count = var.create_waf && var.create_web_acl_association && length(var.resource_arn) > 0 ? length(var.resource_arn) : 0
